...
bool mysql_create_or_drop_trigger(THD *thd, TABLE_LIST *tables, bool create)
{
  ...
  */
  ...
  /*
     If opt_rds_permission_control is on,we allow Super users and reserved users to create triggers on tables in the 'mysql' schema.
   */
  if(opt_rds_permission_control)
  {
    if(check_global_access_noerr(thd,SUPER_ACL) && !is_rds_reserved_user(thd->security_context()->user().str))
    {
