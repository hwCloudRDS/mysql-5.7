...
#include "rds_permission_control.h"    /* is_rds_reserved_user */
                 ...
                 bool extra_port_connection)
{
  ...
  */
  ...
  /* Only allow the reserved user to use the extra port to connect to MySQL */
    if(command == COM_CONNECT &&
       true == extra_port_connection &&
      (!is_rds_reserved_user(thd->security_context()->user().str)))
  {
    ...
    my_error(ER_ACCESS_DENIED_ERROR, MYF(0),
             mpvio.auth_info.user_name,
             mpvio.auth_info.host_or_ip,
			 mpvio.auth_info.password_used ? ER(ER_YES) : ER(ER_NO));
    DBUG_RETURN(1);
  }
