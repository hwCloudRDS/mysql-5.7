                          ...
                          const char *new_password, size_t new_password_len)
{
                     ...
                     sctx->priv_host().str)))
  {
    ...
    if(opt_rds_permission_control && check_reserved_user_db(thd, user, "mysql"))
      return(1);
                     ...
                     char *new_password)
{
  ...
  */
  ...
  /* The logic inside replace_user_table() could have resized/re-allocated
   * the global acl_users array, so we need to refresh the cached acl_user.
   */
  if (!(acl_user= find_acl_user(host, user, TRUE)))
  {
    mysql_mutex_unlock(&acl_cache->lock);

    // We should be able to find the same user since we're still
    // holding the acl_cache lock, so treat this as an error.
    result= 1;
    my_message(ER_PASSWORD_NO_MATCH, ER(ER_PASSWORD_NO_MATCH), MYF(0));
    goto end;
  }
  ...
  */
  ...
  while ((tmp_user_from= user_list++))
  {
    ...
    */
    ...
    /* Look up the user again since the global acl_users array could have
     * been resized and relocated inside the logic of replace_user_table(),
     * in which case the cached pointer acl_user would be pointing to freed
     * and invalid memory, so we need to refresh acl_user.
     */
    if (!(acl_user= find_acl_user(user_from->host.str,
                                  user_from->user.str, TRUE))) {
      // We should be able to find the same user since we're still
