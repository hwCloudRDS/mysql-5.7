                       ...
                       uint *key_count)
{
  ...
  if (check_engine(thd, db, table_name, create_info, !internal_tmp_table))
    DBUG_RETURN(TRUE);
                       ...
                       Alter_info *alter_info)
{
  ...
  */
  ...
  if (check_engine(thd, alter_ctx.new_db, alter_ctx.new_name, create_info, 0))
    DBUG_RETURN(true);
...
/**
  ...
  @param check_disabled_se_list Check the disabled storage engine list.
                         ...
                         bool check_disabled_se_list)
{
  ...
  if (!(*new_engine= ha_checktype(thd, ha_legacy_type(req_engine),
                                  no_substitution, 1, check_disabled_se_list)))
    DBUG_RETURN(true);
