...
static Sys_var_ulong Sys_extra_max_connections(
       ...
       VALID_RANGE(1, 100), DEFAULT(EXTRA_MAX_CONNECTIONS_DEFAULT), BLOCK_SIZE(1),
       NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0));
...
static Sys_var_mybool Sys_rds_permission_control(
       "rds_permission_control",
       "This option indicates whether the RDS Service access control is enabled.",
       GLOBAL_VAR(opt_rds_permission_control),
       CMD_LINE(OPT_ARG), DEFAULT(TRUE));

static bool check_rds_reserved_users(sys_var *self, THD *thd, set_var *var)
{
  if(!var->value)
	return false;
  ...
  if(!is_valid_rds_reserved_users(var->save_result.string_value.str,var->save_result.string_value.length))
  {
...
static bool update_rds_reserved_users_hash(sys_var *self, THD *thd, enum_var_type type)
{
  return (0 != rds_reserved_users_update(rds_reserved_users_ptr) ? true : false);
}
...
static Sys_var_int32 Sys_combine_engine_log_flush_threshold(
       "combine_engine_log_flush_threshold",
       "The minimum number of additional threads the last binlog "
       "group flush stage must have more than the current group size of "
       "the engine log flush stage in order to flush the engine log "
       "and binlog together in a single stage.",
       GLOBAL_VAR(combine_engine_log_flush_threshold),
       CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(0, 16384), DEFAULT(4), BLOCK_SIZE(1),
       NO_MUTEX_GUARD, NOT_IN_BINLOG);
