  ...
  */
  ...
  if (*(head+61) &&
      !(share->default_part_db_type= 
        ha_checktype(thd, (enum legacy_db_type) (uint) *(head+61), 1, 0, 0)))
    goto err;
  ...
  */
  if (legacy_db_type > DB_TYPE_UNKNOWN && 
    ...
    share->db_plugin= ha_lock_engine(NULL, 
                                     ha_checktype(thd, legacy_db_type,
                                                  0, 0, 0));
    ...
    if (next_chunk + 2 < buff_end)
    {
               ...
               !strncmp((char *) next_chunk + 2, "partition", 9))
      {
        ...
        */
        ...
        if (!ha_checktype(thd, DB_TYPE_PARTITION_DB, true, false, false))
        {
          error= 8;
  		...
  		HA_CREATE_INFO *create_info, uint keys, KEY *key_info)
{
                               ...
                               name, CREATE_MODE, create_flags, MYF(0))) >= 0)
  {
    ...
    fileinfo[3]= (uchar) ha_legacy_type(
          ha_checktype(thd,ha_legacy_type(create_info->db_type),0,0,0));
    fileinfo[4]=1;
