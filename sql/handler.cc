...
/**
  Check whether the storage engine is on the disabled storage engine list and
  if it is compiled in.
  If it is not enabled then try to substitute the default engine in.
*/
handlerton *ha_checktype(THD *thd, enum legacy_db_type database_type,
                         bool no_substitute, bool report_error,
                         bool check_disabled_se_list)
{
  handlerton *hton= ha_resolve_by_legacy_type(thd, database_type);
  if (check_disabled_se_list && ha_is_storage_engine_disabled(hton))
  {
    const char *disabled_engine_name= ha_resolve_storage_engine_name(hton);
    if (no_substitute)
    {
      if (report_error)
      {
        my_error(ER_DISABLED_STORAGE_ENGINE, MYF(0), disabled_engine_name);
      }
      return NULL;
    }
  ...
  else if (ha_storage_engine_is_enabled(hton))
  {
    return hton;
  }
  ...
  case DB_TYPE_MRG_ISAM:
    hton = ha_resolve_by_legacy_type(thd, DB_TYPE_MRG_MYISAM);
    break;
  default:
  ...
  if (check_disabled_se_list && ha_is_storage_engine_disabled(hton))
  {
    if (report_error)
    {
      const char *new_engine_name= ha_resolve_storage_engine_name(hton);
      my_error(ER_DISABLED_STORAGE_ENGINE, MYF(0), new_engine_name);
    }
    return NULL;
  }
