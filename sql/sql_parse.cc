...
end_with_restore_list:
  ...
  */
  ...
  case SQLCOM_RENAME_TABLE:
  {
    ...
    {
      if(opt_rds_permission_control && check_mysql_db_tables(thd, table, TRUE, FALSE))
        goto error;
      if(opt_rds_permission_control && check_performance_schema_db_tables(thd, table, TRUE, FALSE))
        goto error;
  ...
  case SQLCOM_DROP_TABLE:
  {
    ...
    {
      if(opt_rds_permission_control && check_mysql_db_tables(thd, all_tables, FALSE, FALSE))
        goto error;
      if(opt_rds_permission_control && check_performance_schema_db_tables(thd, all_tables, FALSE, FALSE))
        goto error;
  ...
  case SQLCOM_DROP_DB:
  {
    if (check_and_convert_db_name(&lex->name, FALSE) != IDENT_NAME_OK)
      break;
    if(opt_rds_permission_control && check_internal_db(thd, lex->name.str))
      break;
    /*
      If in a slave thread :
  ...
  {
    if (opt_rds_permission_control && check_reserved_user(thd, lex->users_list, "CREATE USER"))
      break;
  ...
  {
    if (opt_rds_permission_control && check_reserved_user(thd, lex->users_list, "DROP USER"))
      break;
  ...
  {
    if (opt_rds_permission_control && check_reserved_user(thd, lex->users_list, "RENAME USER"))
      break;
  ...
  {
    if (opt_rds_permission_control && check_reserved_user(thd, lex->users_list, "REVOKE"))
      break;
    if (check_access(thd, UPDATE_ACL, "mysql", NULL, NULL, 1, 1) &&
        check_global_access(thd,CREATE_USER_ACL))
      break;
  ...
  case SQLCOM_GRANT:
  {
    if (opt_rds_permission_control && check_reserved_user(thd, lex->users_list, "GRANT"))
      break;
  ...
  case SQLCOM_RESET:
    ...
    */
    ...
    if(opt_rds_permission_control && !is_rds_reserved_user(thd->security_context()->user().str) && check_global_access_noerr(thd,SUPER_ACL))
    {
  ...
  case SQLCOM_ALTER_TABLE:
  {
	...
	if(opt_rds_permission_control &&
	   ...
	   check_performance_schema_db_tables(thd, all_tables, FALSE, FALSE))
	{
	  res= TRUE;
	  break;
	}
  ...
  {
    if (opt_rds_permission_control && check_reserved_user(thd, lex->users_list, "ALTER USER"))
      break;
    LEX_USER *user, *tmp_user;
