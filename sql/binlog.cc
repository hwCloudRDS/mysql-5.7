...
bool is_create_select_stmt(const LEX *lex)
{
  return (lex->sql_command == SQLCOM_CREATE_TABLE &&
          !(lex->create_info.options & HA_LEX_CREATE_TMP_TABLE) &&
          lex->select_lex->item_list.elements);
}

/*
  CREATE TABLE..SELECT statements are handled differently when using GTIDs
*/
bool is_create_select_gtid(const THD *thd)
{
  return (is_create_select_stmt(thd->lex) &&
          (get_gtid_mode(GTID_MODE_LOCK_NONE) >= GTID_MODE_ON_PERMISSIVE) &&
          (thd->variables.gtid_next.type == AUTOMATIC_GROUP));
}

bool is_gtid_unsafe_stmt_allowed(const THD *thd)
{
  /*
    Some statments are unsafe when using GTIDs such as CREATE TABLE ... SELECT.
    However it can be safe if using row based logging.
  */
  return ((thd->variables.gtid_next.type == AUTOMATIC_GROUP) &&
          (thd->is_current_stmt_binlog_format_row() ||
           thd->variables.binlog_format == BINLOG_FORMAT_MIXED));
}

/*
  Assigns a GTID to the given THD.
  The caller needs to release the global sid lock and locked sidno.

  @param thd Thread to assign to.
  @param is_global_sid_locked Indicates if the global sid lock is already held.
  @param locked_sidno Currently locked sidno.

  @retval false Success.
  @retval true Error.
*/
static bool assign_gtid_to_thd(THD *thd,
                               bool &is_global_sid_locked,
                               rpl_sidno &locked_sidno)
{
  ...
  if (thd->variables.gtid_next.type == AUTOMATIC_GROUP)
  {
    ...
    Rpl_transaction_ctx *ctx=
      thd->get_transaction()->get_rpl_transaction_ctx();
    int ret= gtid_state->generate_automatic_gtid(
      thd, ctx->get_sidno(), ctx->get_gno(), &locked_sidno);
  ...
  {
    DBUG_PRINT("info",
               ("thd->variables.gtid_next.type=%d thd->owned_gtid.sidno=%d",
...
/*
  Assign a GTID as a one-off.

  @param head Thread to assign to.

  @retval false Success.
  @retval true Error.
*/
static bool assign_single_gtid(THD *thd)
{
  bool is_global_sid_locked= false;
  rpl_sidno locked_sidno= 0;
  bool err= assign_gtid_to_thd(thd, is_global_sid_locked, locked_sidno);
  if (locked_sidno > 0)
  {
...
public:
  ...
  */
  ...
  int flush(THD *thd, my_off_t *bytes_written, bool *wrote_xid)
  {
    ...
    if (all_finalized() &&
        is_create_select_gtid(thd) &&
        is_gtid_unsafe_stmt_allowed(thd))
    {
      /*
        stmt_cache and trx_cache are both not empty. This can happen for create
        table select. Each cache flush needs to generate its own gtid. Force
        this by clearing the gtid after the stmt cache flush.
      */
      clear_gtid= true;
    }
...
/*
  Assign GTIDs to members of the flush list.
  Holds the sid locks instead of grabbing/releasing for each.

  A subgroup ends if a double-cache statement is found. Double-cache statements
  need two contiguous GTID values and the second is generated between the cache
  flushes instead of here. Therefore this function must be called for each
  subgroup.

  @param head Thread at the start of this (sub)group.
  @param next_subgroup Points to the value of next_to_commit if a double-cache
                       statement is found.

  @retval false Success.
  @retval true Error.
*/
bool MYSQL_BIN_LOG::assign_gtids_to_subgroup(THD *head, THD **next_subgroup)
{
  DBUG_ENTER("MYSQL_BIN_LOG::assign_gtids_to_subgroup");
  bool error= false;
  bool is_global_sid_locked= false;
  rpl_sidno locked_sidno= 0;

  for ( ; head ; head = head->next_to_commit)
  {
    if (assign_gtid_to_thd(head, is_global_sid_locked, locked_sidno))
    {
      error= true;
    }
    if (thd_get_cache_mngr(head)->all_finalized())
    {
      /* Found a double-cache statement. */
      *next_subgroup= head->next_to_commit;
      break;
    }
  }
                               ...
                               Binlog_event_writer *writer)
{
  ...
  /*
    ...
    assign_gtid_to_thd(), usually during assign_gtids_to_subgroup()
  */
  DBUG_ASSERT(thd->owned_gtid.sidno == THD::OWNED_SIDNO_ANONYMOUS ||
                                   ...
                                   bool need_engine_log_flush)
{
  ...
  /*
    In the case where this thread is a follower and it has done
    engine log flush already but the stage leader (or its subsequently
    leader-turned-follower threads) has registered for an engine
    log flush, I *think* it's safe to skip the flush since this thread's
    flush should have covered up to the LSNs requested by these previous
    threads (because the flush logic always gets the latest/largest
    LSN to flush to), even if their LSNs are larger than this thread's.
    However, to be on the safe side, we'll not overwrite a registered
    flush request. It should only add a small cost (log_sys->mutex) even
    if s the flush request is redundant and results in a no-op.
  */
  if (!m_flush_engine_log)
    m_flush_engine_log = need_engine_log_flush;
  ...
  *m_last= first;
  ...
  if (empty)
  {
    ...
    {
      first->stage_cond_id= m_cond_index++;

      if (m_cond_index >= MAX_STAGE_COND)
        m_cond_index= 0;
    }
  }
  else
...
Stage_manager::enroll_for(StageID stage, THD *thd, mysql_mutex_t *stage_mutex,
                          bool register_engine_log_flush)
{
  ...
  bool engine_log_flush = false;

  // Engine log (eg, REDO log) flush registration applies only
  // to FLUSH_BINLOG_STAGE.
  if (Stage_manager::FLUSH_BINLOG_STAGE == stage)
    engine_log_flush = register_engine_log_flush;

  bool leader= m_queue[stage].append(thd, &slot, engine_log_flush);

  DBUG_ASSERT(slot != UNDEF_COND_SLOT);

#ifdef HAVE_REPLICATION
  if (stage == FLUSH_ENGINE_STAGE && has_commit_order_manager(thd))
  {
  ...
  */
  ...
  switch (stage)
  {
  case Stage_manager::FLUSH_ENGINE_STAGE:
  case Stage_manager::FLUSH_BINLOG_STAGE:
  ...
  */
  if (!leader)
  {
    ...
    */
    ...
    if (thd->get_transaction()->m_flags.pending)
    {
    ...
    /*
      If the follower thread was a leader in the previous stage,
      then very likely (though not guaranteed) it used a different
      slot ID than the current stage leader's, and as such its
      own followers need to be signaled on that slot ID.
    */
    if (thd->stage_leader)
    {
...
THD *Stage_manager::Mutex_queue::fetch_and_empty(int32 *out_sizeP,
                                                 bool *need_engine_log_flush)
{
  ...
  if (out_sizeP)
  {
      *out_sizeP = m_size; // We locked the queue so this is safe.
  }

  if (need_engine_log_flush)
  {
      *need_engine_log_flush = m_flush_engine_log;
  }
...
void Stage_manager::signal_done(THD *queue)
{
    ...
    while(node)
    {
      ...
      /*
        Since we're not holding any mutex here, need to add a
        compile-time barrier to prevent the compiler from
        re-ordering the instructions for setting the thread's
        pending flag to false BEFORE we cache the node->prev_to_commit
        pointer in the code above (since after the flag is set
        to false the thread could be unblocked immediately and
        reset the pointer to NULL).
      */
      barrier();
      node->get_transaction()->m_flags.pending= false;
...
void MYSQL_BIN_LOG::cleanup()
{
  ...
  if (inited)
  {
    ...
    mysql_mutex_destroy(&LOCK_flush_engine);
...
void MYSQL_BIN_LOG::init_pthread_objects()
{
  ...
  mysql_mutex_init(m_key_LOCK_flush_engine, &LOCK_flush_engine, MY_MUTEX_INIT_FAST);
  mysql_cond_init(m_key_update_cond, &update_cond);
  mysql_cond_init(m_key_prep_xids_cond, &m_prep_xids_cond);
  stage_manager.init(
#ifdef HAVE_PSI_INTERFACE
                   m_key_LOCK_flush_engine_queue,
                   m_key_LOCK_flush_binlog_queue,
  ...
  if (log_state.atomic_get() == LOG_OPENED)
  {
    ...
    if ((exiting & LOG_CLOSE_STOP_EVENT) != 0)
    {
      ...
      */
      ...
      bytes_written+= s.common_header->data_written;
      if (flush_io_cache(&log_file)) {
        sql_print_warning("Unable to flush I/O cache on log (file '%s')",
                          log_file_name);;
      }
      update_binlog_end_pos();
    }
...
/**
  Execute the engine (redo-log) flush stage.
*/
void
MYSQL_BIN_LOG::process_flush_engine_stage_queue(THD *first)
{
  mysql_mutex_assert_owner(&LOCK_flush_engine);

  DBUG_PRINT("debug",
             ("MYSQL_BIN_LOG::process_flush_engine_stage_queue called"));

  DBUG_ASSERT(first != NULL);
  DEBUG_SYNC(first, "process_flush_engine_log_stage_as_leader");
...
/**
  Execute the binlog flush stage.
  ...
  @param first is the THD passed from the caller.
  @return Error code on error, zero on success
 */

int
MYSQL_BIN_LOG::process_flush_binlog_stage_queue(my_off_t *total_bytes_var,
                                         bool *rotate_var,
                                         THD *first,
                                         bool flush_engine_log)
{
  DBUG_ENTER("MYSQL_BIN_LOG::process_flush_binlog_stage_queue");
  #ifndef DBUG_OFF
  // number of flushes per group.
  int no_flushes= 0;
  #endif
  DBUG_ASSERT(total_bytes_var && rotate_var && first);
  my_off_t total_bytes= 0;
  int flush_error= 1;
  mysql_mutex_assert_owner(&LOCK_log);

  DEBUG_SYNC(first, "process_binlog_flush_stage_as_leader");

  /*
    If necessary, do an explicit transaction log group write before
    flushing binary log cache to file.
    Note that the entire flush queue ("first") was fetched and emptied
    before this function is invoked, so the next batch would have a new
    leader; and since this was done before ha_flush_logs() is called,
    it's guaranteed to flush prepared records of transactions before
    flushing them to binary log, which is required by crash recovery.
  */
  DBUG_PRINT("debug", ("flush_engine_log: %s", YESNO(flush_engine_log)));
  if (flush_engine_log) {
    DEBUG_SYNC(first, "before_flush_engine_log_in_binlog_stage");

    ha_flush_logs(NULL, true);

    DBUG_EXECUTE_IF("crash_after_flush_engine_log_in_binlog_stage",
                    DBUG_SUICIDE(););
  }

  /*
   Flush thread caches to binary log.
   GTID assigning and flushing is done in subgroups.
   Each subgroup ends when a double-cache statement is found.
   For example a CREATE TABLE..SELECT is a double-cache statement.
  */
  THD *head= first;
  while (head)
  {
    THD* next_subgroup= NULL;
    assign_gtids_to_subgroup(head, &next_subgroup);
    for ( ; head != next_subgroup ; head = head->next_to_commit)
    {
                            ...
                            bool flush_engine_log)
{
  ...
  /*
    ...
    queued.
  */
  if (!stage_manager.enroll_for(stage, queue, leave_mutex, flush_engine_log))
  {
...
int MYSQL_BIN_LOG::ordered_commit(THD *thd, bool all, bool skip_commit)
{
  ...
  */
  ...
  /*
    Stage #0: flushing engine log

    This must be done before flushing the binlog.
  ...
  */
  ...
  DEBUG_SYNC(thd, "waiting_to_enter_flush_engine_stage");
  if (change_stage(thd, Stage_manager::FLUSH_ENGINE_STAGE, thd, NULL,
                   &LOCK_flush_engine))
  {
    DBUG_PRINT("return", ("Thread ID: %u, commit_error: %d",
                          thd->thread_id(), thd->commit_error));
  ...
  DEBUG_SYNC(thd, "waiting_in_the_middle_of_flush_engine_stage");

  // Grab the entire engine-log flush stage queue
  int32 queue_size= 0;
  THD *stage_queue=
              stage_manager.fetch_queue_for(Stage_manager::FLUSH_ENGINE_STAGE,
                                            &queue_size, NULL);

  /*
    Optionally flush engine log. It may be more benificial to delay the flushing
    to stage #1
  */
  bool engine_log_flush = true;

  /*
    We don't need the read-and-check on m_last_group_flush_size to be atomic,
    just need it to be a consistent integer.
    We will not do engine log flush as a separate stage if we think we can
    group "flush_engine_log_stage_threshold" more threads by skipping it, based
    on what the last group size at binlog flush stage was.
  */
  uint last_binlog_grp_flush_sz =
              (uint)my_atomic_load32(&m_last_group_flush_size);
  if ((0 != combine_engine_log_flush_threshold)
      &&
      ((queue_size + combine_engine_log_flush_threshold) >=
                                           last_binlog_grp_flush_sz))
  {
     (void)process_flush_engine_stage_queue(stage_queue);
     engine_log_flush = false; // No need for another engine log flush
                               // in binlog flush stage.
  ...
  */
  ...
  DEBUG_SYNC(thd, "waiting_to_enter_flush_binlog_stage");
  if (change_stage(thd, Stage_manager::FLUSH_BINLOG_STAGE, stage_queue,
                   &LOCK_flush_engine, &LOCK_log, engine_log_flush))
  {
    DBUG_PRINT("return", ("Thread ID: %u, commit_error: %d",
                          thd->thread_id(), thd->commit_error));
    DBUG_RETURN(finish_commit(thd));
  ...
  DEBUG_SYNC(thd, "waiting_in_the_middle_of_binlog_flush_stage");

  // m_last_group_flush_size will be updated atomically in fetch_queue_for().
  stage_queue= stage_manager.fetch_queue_for(Stage_manager::FLUSH_BINLOG_STAGE,
                                             &m_last_group_flush_size,
                                             &engine_log_flush);
  bool update_binlog_end_pos_after_sync = false;
  if (unlikely(!is_open()))
  {
  ...
  flush_error= process_flush_binlog_stage_queue(&total_bytes, &do_rotate,
                                                stage_queue, engine_log_flush);
  ...
  */
  ...
  if (change_stage(thd, Stage_manager::SYNC_STAGE, stage_queue, &LOCK_log, &LOCK_sync))
  {
    DBUG_PRINT("return", ("Thread ID: %u, commit_error: %d",
                          thd->thread_id(), thd->commit_error));
    DBUG_RETURN(finish_commit(thd));
  ...
  */
  ...
  stage_queue= stage_manager.fetch_queue_for(Stage_manager::SYNC_STAGE, NULL, NULL);
...
commit_stage:
      ...
      (sync_error == 0 || binlog_error_action != ABORT_SERVER))
  {
    if (change_stage(thd, Stage_manager::COMMIT_STAGE,
                     stage_queue, leave_mutex_before_commit_stage,
                     &LOCK_commit))
    {
      DBUG_PRINT("return", ("Thread ID: %u, commit_error: %d",
                            thd->thread_id(), thd->commit_error));
    ...
    THD *commit_queue= stage_manager.fetch_queue_for(Stage_manager::COMMIT_STAGE, NULL, NULL);
    DBUG_EXECUTE_IF("semi_sync_3-way_deadlock",
  ...
  else
  {
    ...
    if (flush_error == 0 && sync_error == 0)
      sync_error= call_after_sync_hook(stage_queue);
  }
  ...
  */
  ...
  stage_manager.signal_done(stage_queue);
...
bool THD::is_ddl_gtid_compatible()
{
  ...
  if (is_create_select_stmt(lex))
  {
    if (is_gtid_unsafe_stmt_allowed(this))
    {
      DBUG_RETURN(true);
    }
    /*
