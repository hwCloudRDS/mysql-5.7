...
void Binlog_sender::init()
{
  ...
  if (m_packet.mem_realloc(PACKET_MIN_SIZE)) {
    set_error(ER_OUTOFMEMORY,
              "Failed to reallocate packet buffer");
    DBUG_VOID_RETURN;
  }
...
void Binlog_sender::run()
{
  ...
  if (my_memset(&log_cache, sizeof(log_cache), 0, sizeof(log_cache),
                sql_print_error) == NULL) {
    /* This should not fail - Required by coverity and csec check */
    sql_print_error("Failed to initial the log_cache object");
    /* It is safe to proceed: open_binlog_file will set it */
  }
  ...
  while (!has_error() && !m_thd->killed)
  {
    /*
      ...
      was already sent. The slave then obtains an extra rotation and records
    ...
    */
    ...
    if (unlikely(set_last_file(log_file))) {
      break;
    }
                                     ...
                                     my_off_t log_pos)
{
  ...
  */
  ...
  size_t max_size= (size_t) event_len
                    - LOG_EVENT_HEADER_LEN - Binary_log_event::ROTATE_HEADER_LEN;
  if (my_memcpy(rotate_header + Binary_log_event::ROTATE_HEADER_LEN, 
                max_size, p, ident_len, sql_print_error) == NULL) {
    DBUG_RETURN(1); /* rotate_header is not NULL */
  }
                                               ...
                                               bool *found)
{
  ...
  *found= false;
  ...
  if (my_memset(buf, LOG_EVENT_HEADER_LEN, 0, LOG_EVENT_HEADER_LEN,
                sql_print_error) == NULL) {
    /* This should not fail - Required by coverity and csec check */
    sql_print_error("Failed to initial buffer for GTID event");
    return 1;
  }
...
int Binlog_sender::send_heartbeat_event(my_off_t log_pos)
{
  ...
  size_t max_size= (size_t) event_len - LOG_EVENT_HEADER_LEN;
  if (my_memcpy(header + LOG_EVENT_HEADER_LEN, 
                max_size, p, ident_len, sql_print_error) == NULL) {
    DBUG_RETURN(1); /* header is not NULL */
  }
...
inline int Binlog_sender::set_last_file(const char *log_file)
{
  char* ret_ptr= my_strcpy(m_last_file_buf, FN_REFLEN,
                           log_file, sql_print_error);
  if (ret_ptr == NULL) {
    sql_print_error("Failed to copy log name %s", log_file);
    return 1;
  }
