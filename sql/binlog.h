...
public:
  ...
  public:
    Mutex_queue()
      : m_first(NULL), m_last(&m_first), m_last_thd(NULL),
        m_cond_index(0), m_size(0), m_flush_engine_log(false)
    {
    }
    ...
    */
    bool append(THD *first, int *slot, bool check_engine_log_flush);
    ...
    */
    THD *fetch_and_empty(int32 * out_sizeP, bool* need_engine_log_flush);
  ...
  private:
    ...
    */
    ...
    /*
      flag to indicate whether to flush engine log for threads
      in the queue -- only applicable to FLUSH_BINLOG_STAGE
      stage.
    */
    bool m_flush_engine_log;
...
public:
  ...
  void init(
#ifdef HAVE_PSI_INTERFACE
            PSI_mutex_key key_LOCK_flush_engine_queue,
            PSI_mutex_key key_LOCK_flush_binlog_queue,
...
#endif
    m_queue[FLUSH_ENGINE_STAGE].init(
#ifdef HAVE_PSI_INTERFACE
                              key_LOCK_flush_engine_queue
#endif
                              );
    m_queue[FLUSH_BINLOG_STAGE].init(
#ifdef HAVE_PSI_INTERFACE
                              key_LOCK_flush_binlog_queue
#endif
                              );
  ...
  */
   ...
   */
  THD *fetch_queue_for(StageID stage, int32 * out_sizeP, bool *need_engine_log_flush) {
    DBUG_PRINT("debug", ("Fetching queue for stage %d", stage));
    return m_queue[stage].fetch_and_empty(out_sizeP, need_engine_log_flush);
  }
  ...
  void enter_cond_slot(int slot)
  {
    ...
    (void) mysql_cond_timedwait(&(m_cond_done[slot]), &(m_lock_done[slot]), &abstime);
  }
...
class MYSQL_BIN_LOG: public TC_LOG
{
  ...
  PSI_mutex_key m_key_LOCK_flush_engine_queue;
  PSI_mutex_key m_key_LOCK_flush_binlog_queue;
  ...
  /** The instrumentation key to use for @ LOCK_flush_engine. */
  PSI_mutex_key m_key_LOCK_flush_engine;
  ...
  mysql_mutex_t LOCK_flush_engine;
...
public:
  ...
  */
  ...
  void set_psi_keys(PSI_mutex_key key_LOCK_index,
                    ...
                    PSI_mutex_key key_LOCK_flush_engine,
                    PSI_cond_key key_COND_done,
                    ...
                    PSI_file_key key_file_log_index_cache)
  {
    ...
    m_key_LOCK_flush_engine_queue= key_LOCK_flush_engine_queue;
    m_key_LOCK_flush_binlog_queue= key_LOCK_flush_binlog_queue;
    ...
    m_key_LOCK_flush_engine= key_LOCK_flush_engine;
...
private:
  ...
  bool change_stage(THD *thd, Stage_manager::StageID stage,
                    ...
                    mysql_mutex_t *enter, bool flush_engine_log=false);
  ...
  void process_flush_engine_stage_queue(THD *first);
  int process_flush_binlog_stage_queue(my_off_t *total_bytes_var, bool *rotate_var,
                                       THD *first, bool flush_engine_log);
...
public:
  ...
  */
  ...
  /**
    Assign automatic generated GTIDs for all commit group threads, up to a
    double-cache statement (where both stmt and trx caches are finalized), in
    the flush stage having gtid_next.type == AUTOMATIC_GROUP.

    @param head The first thread of this subgroup.
    @param next_subgroup The next thread after this subgroup.
    @return Returns false if succeeds, otherwise true is returned.
  */
  bool assign_gtids_to_subgroup(THD *head, THD **next_subgroup);
