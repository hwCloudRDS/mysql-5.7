...
//copy from percona 5.7 includ/my_sys.h
#define my_microsecond_getsystime()    (my_getsystime()/10)

/** Maximum number of native events a listener can read in one go */
/* previous value is 1024, ev will exceeds the max single use of 10000 bytes of stack use */
/* change it from 1024 to 800 to follow coverity check rule */
...
struct connection_t
{
  ...
  uint tickets;
  ulonglong enqueue_time; /* record the time to enter the common queue */

  bool is_logged_in() const
  {
     return (conn_flags & CONNECTION_LOGGED_IN);
  }
  void set_logged_in()
  {
     conn_flags |= CONNECTION_LOGGED_IN;
  }
  ...
  return (mode == TP_HIGH_PRIO_MODE_STATEMENTS) ||
    (mode == TP_HIGH_PRIO_MODE_TRANSACTIONS && c->tickets > 0 &&
     (thd_is_transaction_active(c->thd) ||
      ...
      //TODO huafengchun: add this feature later   c->thd->backup_tables_lock.is_acquired() ||
      //TODO huafengchun: add this feature later   c->thd->backup_binlog_lock.is_acquired() ||
...
static void check_stall(thread_group_t *thread_group)
{
  ...
  /* add prio_kickup_timer check in the below for loop */
  /* we boost connections from low priority queue to high priority queue if it waits too long */
  connection_t *c;
  for(;;)
  {
     c=thread_group->queue.front();
     if( c && (pool_timer.current_microtime - c->enqueue_time > threadpool_prio_kickup_timer*1000ULL ) )
     {
       thread_group->queue.remove(c);
...
static void start_timer(pool_timer_t* timer)
{
  ...
  error=mysql_thread_create(key_timer_thread,&thread_id, NULL, timer_thread, timer);
  if(error){
    ...
    */
    for(int i=(listener_picks_event)?1:0; i < cnt ; i++)
    {
      ...
      else
      {
        ...
        /* add enqueue time of this connection to the common queue */
        c->enqueue_time = my_microsecond_getsystime();
...
static void queue_put(thread_group_t *thread_group, connection_t *connection)
{
  ...
  // add enqueue time of this connection to common queue
        ...
        */
        ...
        else if (too_many_busy_threads(thread_group))
        {
          ...
          */
          ...
          connection->tickets= threadpool_high_prio_tickets;
          /* add enqueue time of the connection to this common queue */
          connection->enqueue_time = my_microsecond_getsystime();
          thread_group->queue.push_back(connection);
          connection= NULL;
        }
...
static inline void wait_begin(thd_wait_type wait_type, thread_group_t *thread_group)
{
  ...
  if (!queues_are_empty(thread_group) || (!thread_group->listener && thread_group->connection_count > 1))
  {
    /* 
...
static inline void wait_end(thd_wait_type wait_type, thread_group_t *thread_group)
{
  DBUG_ENTER("wait_end");
  mysql_mutex_lock(&thread_group->mutex);
  thread_group->active_thread_count++;
...
bool add_connection(Channel_info *channel_info)
{
  ...
  thd->scheduler= &thd_event_functions;
...
void tp_wait_begin(THD *thd, int type)
{
  ...
  DBUG_ASSERT( type > THD_WAIT_NONE && type < THD_WAIT_LAST );
  ...
  if (connection)
  {
    ...
    DBUG_ASSERT( THD_WAIT_NONE == connection->wait_type );
...
{ 
  ...
  if (connection)
  {
    DBUG_ASSERT(THD_WAIT_NONE != connection->wait_type && 0 == connection->num_nested_waits);
    ...
    {
       connection->num_nested_waits--;
       DBUG_ASSERT(connection->num_nested_waits >= 0);
    }
  }
  DBUG_VOID_RETURN;
}
...
bool tp_init()
{
  ...
  if(group_count != threadpool_size)
  {
