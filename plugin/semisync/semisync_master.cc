                             ...
                             ulong packet_len)
{
  ...
  if (my_strncpy(log_file_name, FN_REFLEN, 
                 (const char*)packet + REPLY_BINLOG_NAME_OFFSET,
                 log_file_len, sql_print_error) == NULL) {
    sql_print_error("Failed to copy log filename");
    goto l_end; /* log_file_name is not NULL */
  }
...
ReplSemiSyncMaster::ReplSemiSyncMaster()
  : active_tranxs_(NULL),
    ...
    state_(0)
{
  /* The folllowing initialization for the string is good
   * - In case we need to cleanup all the coverity check results
   *   we can us my_memset or worse, an loop  */ 
  reply_file_name_[0]= '\0';
  wait_file_name_[0]= '\0';
  commit_file_name_[0]= '\0';

  /* Note: LOCK_binlog_.m_psi would be initialized in initObject
   * - Mutex initialization can only be done after MY_INIT()
   * - Need to ignore the coverity check result for it */
}
					  ...
					  unsigned long size)
{
  ...
  int hlen=0;
  {
     ...
     * is required.
     */
    if (my_memcpy(header, (size_t)size, kSyncHeader, sizeof(kSyncHeader),
                  sql_print_error) == NULL) {
      sql_print_warning("Failed to build packet header for semi-sync extra "
                        "header, semi-sync replication disabled");
      disableMaster();
      return 0;
    }
...
void AckInfo::update(const char *log_file_name, my_off_t log_file_pos)
{
  if (my_strncpy(binlog_name, FN_REFLEN, log_file_name,
                 FN_REFLEN - 1, sql_print_error) == NULL) {
    /* When read out the log_file_name, its length is ensured to be less
     * then FN_REFLEN, otherwise, the error has already been handled - See
     * ReplSemiSyncMaster::reportReplyPacket. So the my_strncpy function
     * call should not fail. MySQL code ensures the name with FN_REFLEN
     * and assume that str function call would not fail, and the callers
     * in the call chain to this function do not have the error handling. 
     *
     * This fix is just for the CSECCHECK. If we want to handle this error,
     * we need to add error handling for serveral call chains, which would
     * increase the complexity. So we just print out an error message here */
    sql_print_error("Failed to copy binlog file name"); 
  } 
  binlog_name[FN_REFLEN - 1]= '\0';
...
int AckContainer::resize(unsigned int size, const AckInfo **ackinfo)
{
  ...
  if (size < 1) {
    sql_print_error("Incorrect value %u for "
                    "rpl_semi_sync_master_wait_for_slave_count", size);
    return -1;
  }
...
void AckContainer::clear()
{
  if (m_ack_array)
  {
    size_t max_size= sizeof(AckInfo) * m_size;
    if (my_memset(m_ack_array, max_size, 0, max_size,
                  sql_print_error) == NULL) {
      sql_print_information("The memset failed for semisync ack array - "
                            "Reset the memory for the semisync ack array "
                            "directory ");
      /* The memset (it is faster) should not fail and this function should
       * not fail - In case reach here (For CESEC-Check only, well). */
      for (unsigned int i= 0; i < m_size; i++) {
        AckInfo* ackinfo= &m_ack_array[i];
        ackinfo->server_id= 0;
        ackinfo->binlog_pos= 0ULL;
        for (int j= 0; j < FN_REFLEN; j++) {
          ackinfo->binlog_name[j]= 0;
        }
      }
    }
    m_empty_slot= m_size;
