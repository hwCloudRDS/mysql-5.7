...
bool Ack_receiver::start()
{
  ...
  if(m_status == ST_DOWN)
  {
    ...
    {
      sql_print_error("Failed to start semi-sync ACK receiver thread: "
                      "Could not initialize thread(errno:%d) attributes", errno);
      m_status= ST_DOWN;
      failed= true;
      goto end;
    }
  ...
  return function_exit(kWho, failed);
}
...
static int init_net(NET *net, unsigned char *buff, unsigned int buff_len)
{
  if (my_memset(net, sizeof(NET), 0, sizeof(NET),
                sql_print_error) == NULL) {
    sql_print_error("Failed to initial the net object");
    return 1;
  }
...
void Ack_receiver::run()
{
  ...
  if (my_memset(net_buff, REPLY_MESSAGE_MAX_LENGTH, 0, REPLY_MESSAGE_MAX_LENGTH,
                sql_print_error) == NULL) {
    sql_print_error("Failed to initial the net_buff object");
    goto end;
  }
  net.buff= NULL; /* Avoiding coverity error? */
  if (unlikely(init_net(&net, net_buff, REPLY_MESSAGE_MAX_LENGTH))) {
    sql_print_information("Failed to start ack receiver thread");
    goto end;
  };
