...
public:
  ...
  {
    m_greatest_ack.server_id= 0;
    m_greatest_ack.binlog_name[0]= '\0';
    m_greatest_ack.binlog_pos= 0ULL;
  }
...
private:
  ...
  */
  ...
  {
    unsigned int i= 0;
    if (m_ack_array == NULL) return;/*Nothing to remove: Required by coverity*/
    for (; i < m_size; i++)
    {
  ...
  */
                             ...
                             my_off_t log_file_pos)
  {
    ...
    if (m_ack_array == NULL) {
      return m_size; /* Not update any (Required by coverity) */
    }
