                                 ...
                                 my_off_t binlog_filepos)
{
  ...
  if (my_memcpy(reply_buffer + REPLY_BINLOG_NAME_OFFSET,
                REPLY_BINLOG_NAME_LEN, 
                binlog_filename, name_len + 1, /* including trailing '\0' */
                sql_print_error) == NULL) {
     reply_res= 1;
     sql_print_error("Semi-sync slave send reply failed: Faile to copy "
                     "binlog filename '%s'", binlog_filename);
     goto l_end;
  }
