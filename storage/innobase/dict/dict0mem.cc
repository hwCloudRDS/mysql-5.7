	...
	ulint		flags2)	/*!< in: table flags2 */
{
    ...
    /*
     * query_cache_inv_id records the last transaction that
     * potentially modifies the table, but it is not persisted
     * to disk. If the table is evicted from dict cache and then
     * reload, the previous query_cache_inv_id is lost.
     * We must assume the worst and set it to max_trx_id to keep
     * query cache function correctly. It is OK to set
     * query_cache_inv_id to max_trx_id even if this is the first
     * time the table is loaded to dict cache.
     */
    if (trx_sys)
    {
        table->query_cache_inv_id = trx_sys_get_max_trx_id();
    }
