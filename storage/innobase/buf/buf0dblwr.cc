	...
	for (ulint i = 0; i < TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * 2; i++) {
        ...
        if (!success) {
                ...
                if (size % UNIV_PAGE_SIZE) {
                        ...
                        if (buf_is_zeroes(recovery_buf, size))
                        {
                           ib::info() << "Parallel doublewrite buffer size "
                                       << size << " bytes is not a multiple of "
                                       "a page size "
                                       << UNIV_PAGE_SIZE << " bytes,"
                                       "but it contains only zeroes";
	...
	ulint dblwr_partition)	/*!< in: doublewrite partition */
{
...
#ifdef UNIV_DEBUG
        {
           buf_pool_t* buf_pool = buf_pool_from_bpage(dblwr_shard->buf_block_arr[0]);
           buf_flush_t flush_type = buf_page_get_flush_type(dblwr_shard->buf_block_arr[0]);

           // Since there are pages in the double write buffer, n_flush can't be 0
           ut_ad(buf_pool->n_flush[flush_type] > 0);

           // Verify that all the pages in the double write buffer belong to the same
           // bufferpool instance and the same flush type
           for (ulint i = 1; i < dblwr_shard->first_free; i++) 
           {
              ut_ad(buf_pool == buf_pool_from_bpage(dblwr_shard->buf_block_arr[i]));
              ut_ad(flush_type == buf_page_get_flush_type(dblwr_shard->buf_block_arr[i]));
           }
        }
#endif
	...
	for (ulint len2 = 0, i = 0;
        ...
        ret = os_file_write(io_req, parallel_dblwr_buf.path, parallel_dblwr_buf.file,
        ...
        if (!ret)
        {
           ...
           */
           ...
           ib::fatal() << "Failed to write to the parallel double-write buffer file "
	...
	blocks. Next do the writes to the intended positions. */
        ...
        free = dblwr_shard->batch_size = dblwr_shard->first_free;
        dblwr_shard->first_free = 0;
        sig_count = os_event_reset(dblwr_shard->batch_completed);
        os_wmb;

        /*
         * dblwr_shard->first_free has been reset.
         * dblwr_shard->batch_completed event has been reset.
         *
         * Do not access dblwr_shard after the loop, except
         * for waiting for the batch_completed event.
         *
         * The reason is that, when this function is called
         * from buf_flush_end(), init_flush is FALSE. As the AIO
         * requests are being submitted in the following loop, n_flush
         * value can start to drop (The IO completion thread decreses
         * n_flush). After the last AIO request is submitted,
         * n_flush can hit 0. Then another thread can
         * potentially pass buf_flush_start() and start changing
         * dblwr_shard.  
         *
         */
        for (ulint i = 0; i < free; i++) {
		buf_dblwr_write_block_to_datafile(
	...
	os_aio_simulated_wake_handler_threads();
        ...
        os_event_wait_low(dblwr_shard->batch_completed, sig_count);
...
try_again:
        ...
        ut_ad( 0 == dblwr_shard->batch_size );
        ut_ad( TRUE == buf_pool_from_bpage(bpage)->init_flush[flush_type] );
	...
	if (dblwr_shard->first_free == srv_doublewrite_batch_size) {
        ...
        const size_t univ_page_size_physical = univ_page_size.physical();
        const size_t buf_offset = (univ_page_size_physical * dblwr_shard->first_free);
	byte* const p = dblwr_shard->write_buf + buf_offset;

        /* dblwr_shard->write_buf has a size of srv_doublewrite_batch_size * UNIV_PAGE_SIZE,
         * see buf_parallel_dblwr_create() */
        const size_t buf_size_remain = srv_doublewrite_batch_size * UNIV_PAGE_SIZE - buf_offset;

	if (bpage->size.is_compressed()) {
                const size_t bpage_physical_size = bpage->size.physical();
		UNIV_MEM_ASSERT_RW(bpage->zip.data, bpage_physical_size);
		/* Copy the compressed page and clear the rest. */

                if (NULL == my_memcpy(p, buf_size_remain, 
                                      bpage->zip.data, bpage_physical_size,
                                      sql_print_error))
                {
                     ib::fatal() << "Cannot copy to parallel dwb shard buffer due to "
                                    "memory security error.";
                }

		if (NULL == my_memset(p + bpage_physical_size, 
                                      buf_size_remain - bpage_physical_size, 
                                      0x0,
		                      univ_page_size_physical - bpage_physical_size,
                                      sql_print_error))
                {
                     ib::fatal() << "Cannot clear parallel dwb shard buffer due to "
                                    "memory security error.";
                }
	} else {
		...
		if ( NULL == my_memcpy(p, buf_size_remain, 
                                       ((buf_block_t*) bpage)->frame, bpage->size.logical(),
                                       sql_print_error))
                {
                     ib::fatal() << "Cannot copy to parallel dwb shard buffer due to "
                                    "memory security error.";
...
{
        ut_ad(!srv_read_only_mode);
        /* The buffer size is one doublewrite batch per buffer pool instance. */
...
buf_parallel_dblwr_create(void)
{
        ...
        if (NULL == my_memset(parallel_dblwr_buf.shard, 
                              ...
                              sql_print_error))
        {
           ib::error() << "Cannot clear parallel dwb shard due to "
                          "memory security error.";
           return (DB_ERROR);
        }
        ...
        recovery_buf_unaligned(NULL)
{
        ...
        if (NULL == my_memset(shard, sizeof(shard), 0, sizeof(shard), sql_print_error))
        {
           ib::fatal() << "Cannot clear parallel dwb shard buffer due to "
                          "memory security error.";
        }
