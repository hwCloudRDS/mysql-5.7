...
os_event_t	buf_flush_event = NULL;
	...
	buf_page_t*	bpage)	/*!< in: pointer to the block in question */
{
	...
	if (buf_pool->n_flush[flush_type] == 0
        ...
        /*
         * Must still be holding mutex to prevent another 
         * thread pass buf_flush_start(), since
         * n_flush can be 0 and init_flush can be FALSE
         * at this point. We can't allow another thread
         * to start a batch flush until buf_dblwr_update 
         * is done because buf_dblwr_update accesses parallel
         * double write buffer
         */
					...
					or BUF_FLUSH_LIST */
{
	...
	if (buf_pool->n_flush[flush_type] == 0) {
		...
		os_event_set(buf_pool->no_flush[flush_type]);
                ...
                // n_flush is 0 implies that there is no page in double write buffer
                ut_ad ( 0 == parallel_dblwr_buf.shard[buf_parallel_dblwr_partition(buf_pool, flush_type)].first_free);


	}
	buf_pool_mutex_exit(buf_pool);

        if (!srv_read_only_mode && need_to_flush)
        {
        ...
        {
           /* n_flush was 0 and init_flush was NULL. As soon as this
            * thread exits the above mutex, another thread can
            * pass buf_flush_start() and add a page to double
            * write buffer partition. We should not call 
            * buf_dblwr_flush_buffered_writes() here; otherwise we can 
            * flush pages that belong to another batch.
            */
	   if (!srv_read_only_mode &&
               (!srv_use_doublewrite_buf || buf_dblwr == NULL) ) {
    ...
    const char *thread_name, const bool report_error)
{
   ...
   if (srv_cleaner_real_time)
   {
      ...
      if (!failure)
      {
        ib::info() << thread_name
                   << " SCHED_RR priority: "
                   << buf_flush_page_cleaner_realtime_pri;
      }
