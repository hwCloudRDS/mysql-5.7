					...
					'/' char, table name */
{
	...
	/* If there are locks on the table, then this transaction cannot
	read/write from/to the cache.

    trx->id is not assigned until the first write statement;
    that is, it doesn't necessarily reflect when the transaction
    starts. So it cannot be used to test what the transaction
    can see. We use read view instead.
