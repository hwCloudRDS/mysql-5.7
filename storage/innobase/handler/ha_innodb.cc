	...
	ulonglong *unused)	/*!< unused for this engine */
{
    ...
    /*
     * 1. In the SERIALIZABLE mode we add LOCK IN SHARE MODE to
     * every plain SELECT if AUTOCOMMIT is not on.
     * 2. UR query result is cached, then RR query can return
     * uncommited data. So we disable caching for uncommited
     * read queries.
     */
	if ((TRX_ISO_SERIALIZABLE == trx->isolation_level) ||
        (TRX_ISO_READ_UNCOMMITTED == trx->isolation_level))
    {
...
{
       if (srv_read_only_mode) {
            ib::error() << "Server debug variable innodb_log_checkpoint_now cannot be "
                    "set when InnoDB is in read-only mode. This is a no-op.";
             return;
        }
						...
						check function */
{
        if (srv_read_only_mode) 
        {
            ib::error() << "Server debug variable innodb_buf_flush_list_now cannot be "
                           "set when InnoDB is in read-only mode. This is a no-op.";
...
static MYSQL_SYSVAR_ULONG(doublewrite_batch_size, srv_doublewrite_batch_size,
  ...
  NULL, NULL, 64, 1, MAX_DOUBLEWRITE_BATCH_SIZE, 0);
...
static MYSQL_SYSVAR_BOOL(cleaner_real_time, srv_cleaner_real_time,
  PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
  "Use real time scheduling for page cleaner threads.",
  NULL, NULL, TRUE);
...
#endif /* UNIV_DEBUG */
  MYSQL_SYSVAR(cleaner_real_time),
  NULL
};
