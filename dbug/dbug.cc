...
struct _db_func_profile_ {
   ulonglong gross_total_time;
   ulonglong net_total_time;
   ulonglong num_calls;

   _db_func_profile_(void): gross_total_time(0),net_total_time(0), num_calls(0)
   {
   };
};

typedef boost::unordered_map<std::string, struct _db_func_profile_> FUNC_PROF_MAP;
...
typedef struct _db_code_state_ {
  ...
  FUNC_PROF_MAP *func_prof_map; /* Remember function profiling result       */
                ...
                uint _line_, struct _db_stack_frame_ *_stack_frame_)
{
  ...
  _stack_frame_->enter_time = 0;
  _stack_frame_->callee_time = 0;
  cs->func=  _func_;
  cs->file=  _file_;
  ...
  /* Do a dirty read of the cs->stack->flag to test if profiling enabled */
  if (PROFILING)
  {
     _stack_frame_->enter_time = my_getsystime();
  }
  else
  {
     read_lock_stack(cs);
     
     _stack_frame_->level |=  framep_trace_flag(cs, _stack_frame_->prev);
...
void _db_return_(uint _line_, struct _db_stack_frame_ *_stack_frame_)
{
  ...
  /* If enter_time is stamped, then count this function call */
  if ( 0 != cs->framep->enter_time )
  {
     ulonglong exit_time = my_getsystime();
     ulonglong elapsed_time = exit_time - cs->framep->enter_time; 

     if (!cs->func_prof_map)
     {
        cs->func_prof_map = new FUNC_PROF_MAP(256);
     }

     if (cs->func_prof_map)
     {
        struct _db_func_profile_ &func_entry = (*cs->func_prof_map)[cs->func];
        func_entry.gross_total_time += elapsed_time;
        func_entry.net_total_time += (elapsed_time - cs->framep->callee_time);
        func_entry.num_calls++;
     }

     if (cs->framep->prev)
     {
        cs->framep->prev->callee_time += elapsed_time;
     }
  }


  /* Only tracing when profiling is not enabled
   */ 
  if (!PROFILING) {
...
static BOOLEAN Writable(const char *pathname)
{
  ...
  else
  {
     ...
     char * pathcopy = DbugMalloc(pathlen+1);
     ...
     if (pathcopy)
     {
        ...
        strncpy(pathcopy, pathname, pathlen);
        pathcopy[pathlen]='\0';
...
void _db_profile_end_(my_thread_id id, CODE_STATE *cs)
{
  ...
  if (cs->func_prof_map)
  {
    ...
    (void) fprintf(cs->stack->out_file, "BEGIN T@%u: function gross_total_time net_total_time num_calls\n", id);

    for ( FUNC_PROF_MAP:: iterator it = cs->func_prof_map->begin();
          cs->func_prof_map->end() != it;
          ++it)
    {
      (void) fprintf (cs->stack->out_file,
                      "T@%u: %s %llu %llu %llu\n",
                      id,
                      it->first.c_str(),
                      it->second.gross_total_time,
                      it->second.net_total_time,
                      it->second.num_calls);

    }

    (void) fprintf(cs->stack->out_file, "END T@%u\n", id);

    DbugFlush(cs);                       /* This does a unlock */

    unlock_stack(cs);
     
    delete cs->func_prof_map;
    cs->func_prof_map = NULL;
  }
}
